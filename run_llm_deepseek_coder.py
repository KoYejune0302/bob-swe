...
output_file = f"results/deepseek-coder/model_patches_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

# More aggressive quantization for 12GB VRAM
...
pipe = pipeline(
    ...
    model=model,
    tokenizer=tokenizer,
    max_new_tokens=1536,  # Slightly shorter context
    temperature=0.3,
    top_k=50
)
